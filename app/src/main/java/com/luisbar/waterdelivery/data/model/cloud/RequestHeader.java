...
public class RequestHeader extends ModelRepository<RequestHeader.RequestHeaderNested> {

    public RequestHeader() {
        super("RequestHeaderNested", "To001s", RequestHeaderNested.class);
    }

    public RestContract createContract() {
        RestContract contract = super.createContract();
//////MArco
        contract.addItem(new RestContractItem("/" + getNameForRestUrl() + "/updateStateTwoToThree", "POST"),
                getClassName() + ".updateStateTwoToThree");

        contract.addItem(new RestContractItem("/" + getNameForRestUrl() + "/saveOrder", "POST"),
                getClassName() + ".saveOrder");

        return contract;
    }

    public void updateStateTwoToThree(int id, String amount, String idrepa, final Adapter.JsonObjectCallback callback) {
        invokeStaticMethod("updateStateTwoToThree",
                ImmutableMap.of("id", id, "amount", "0", "idrepa", idrepa),
                new Adapter.JsonCallback() {
                    @Override
    ...
    public void saveOrder(String clientId, String date, String observation, String products, final Adapter.JsonObjectCallback callback) {
        invokeStaticMethod("saveOrder",
                ImmutableMap.of("clientId", clientId,
                        "date", date, "observation", observation,
                        "products", products),
